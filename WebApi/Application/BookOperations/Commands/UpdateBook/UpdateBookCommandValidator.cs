...
namespace WebApi.Application.BookOperations.Commands.UpdateBook
{
    public class UpdateBookCommandValidator : AbstractValidator<UpdateBookCommand>
    {
        ...
        {
            RuleFor(command => command.BookID).GreaterThan(0).When(x => x.Model.PageCount != 0);
            RuleFor(command => command.Model.AuthorID).GreaterThan(0).When(x => x.Model.PageCount != 0);
